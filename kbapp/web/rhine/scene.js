/*
 * 移植自 RhineLabUI src/scene.ts（75637 字节）
 * 由 kbapp/port-rhine.mjs 用 esbuild 剥类型生成 —— 逻辑与源码逐行一致，请勿手改。
 * 要改行为请改上游源码后重新移植，或在 kbapp/port-overrides/ 下放改写版。
 */
import * as THREE from "three";
import { ArchiveVisibility } from "./archive-visibility.js";
import { InstanceUpdates } from "./instance-updates.js";
import { RenderState } from "./render-state.js";
import { SharedDepthAO, SharedDepthBokeh } from "./shared-depth.js";
import { disposeThreeTree } from "./three-resources.js";
import { ThemeWave } from "./theme-motion.js";
import { themeMaterial, themeEnvironment } from "./theme-material.js";
import { RhythmMotion, rhythmDisplacement, quietBands } from "./archive-play-motion.js";
import { GLTFLoader } from "three/addons/loaders/GLTFLoader.js";
import { createArchiveLighting } from "./archive-lighting.js";
import { EffectComposer } from "three/addons/postprocessing/EffectComposer.js";
import { RenderPass } from "three/addons/postprocessing/RenderPass.js";
import { OutputPass } from "three/addons/postprocessing/OutputPass.js";
import { SMAAPass } from "three/addons/postprocessing/SMAAPass.js";
import { normalizeQuality } from "./render-quality.js";
import { applyTextureQuality, resizeQuality } from "./quality-renderer.js";
import { CardAppearance } from "./appearance.js";
import { configureInternalOptics } from "./internal-optics.js";
import { DecryptionController } from "./decryption.js";
import { fileAtSlot, fileLocation } from "./data.js";
import {
  cellKey,
  sameCell,
  selectionCell,
  ...
  COLUMN_SPACING,
  ROW_SPACING
} from "./archive-loop.js";
import { labelMarkSvg } from "./brand.js";
import { archiveFraming } from "./viewport-layout.js";
import { ArchiveDrag, ArchivePlaneMomentum } from "./archive-drag.js";
import { assetUrl as publicAsset } from "./asset-url.js";
import {
  archiveWave,
  extraction,
...
class ArchiveScene {
  constructor(container, selectionPulse = baselineSelectionWave, deferSelectionPulse = false, lightingLook = "baseline") {
    this.container = container;
    this.selectionPulse = selectionPulse;
    this.deferSelectionPulse = deferSelectionPulse;
    this.lightingLook = lightingLook;
    ...
    this.renderer.setPixelRatio(
      Math.min(devicePixelRatio, 1.5) * Math.min(innerWidth / 1920, innerHeight / 1080)
    );
    ...
    this.light.shadow.bias = lightingLook === "refined" ? -12e-5 : -3e-4;
  ...
  deferSelectionPulse;
  lightingLook;
  inputEvents = new AbortController();
  presence = 1;
  presenceTarget = 1;
  setPresentationVisible(visible, immediate = false) {
  ...
  get presentationHidden() {
    return this.presenceTarget === 0 && this.presence === 0;
  }
  presentationDrop(cell) {
    const delay = 0.15 * (1 + Math.tanh((cell.row - this.selectedCell.row) * 0.1 + (cell.lane - this.selectedCell.lane) * 0.25));
    return 35 * Math.pow(THREE.MathUtils.clamp((1 - this.presence - delay) / 0.7, 0, 1), 2);
  }
  revealImmediately() {
  ...
  theme = new ThemeWave();
  subduedIndex = { value: 0 };
  selectedIndexOnly = false;
  superPerformance = false;
  setSuperPerformance(enabled) {
    if (this.superPerformance === enabled) return;
    this.superPerformance = enabled;
    for (const inst of this.instances) {
      const original = inst.userData.fullMaterial ??= inst.material;
      if (enabled && !inst.userData.fastMaterial) {
        ...
        fast.transmission = 0;
        fast.clearcoat = 0;
        fast.roughness = Math.max(0.45, original.roughness);
  ...
  setSelectedIndexAccent(onlySelected) {
    this.selectedIndexOnly = onlySelected;
  }
  themeAttribute;
  get themeAmount() {
    return this.theme.background(performance.now() / 1e3);
  }
  setTheme(dark, immediate = false) {
    this.theme.set(dark, performance.now() / 1e3, this.selectedCell, immediate);
  }
  playfield = { enabled: false, bands: quietBands(), strength: 1, flatten: 0, target: null, breathing: true };
  flatMix = 0;
  rhythm = new RhythmMotion();
  rhythmStyle = "legacy";
  setRhythmStyle(style) {
    this.rhythmStyle = style;
  }
  relayLifts = /* @__PURE__ */ new Map();
  relayPoints = /* @__PURE__ */ new Map();
  relayActive = false;
  onRelayPick;
  setPlayfield(enabled, bands, strength, flatten, target, breathing = true) {
  ...
  relayCandidates() {
    ...
    return [...this.relayPoints.keys()].filter((key) => {
      ...
      if (x < 0.18 || x > 0.82 || y < 0.32 || y > 0.76) return false;
  ...
  camera = new THREE.PerspectiveCamera(34, 16 / 9, 5, 300);
  composer;
  ao;
  bokeh;
  instances = [];
  ...
  renderedFrames = 0;
  reusedFrames = 0;
  visibility = new ArchiveVisibility();
  instanceCapacity = LOOP_COLUMNS * LOOP_ROWS;
  drawnCells = [];
  extraCoverage = false;
  setArchiveCoverage(extra) {
    this.extraCoverage = extra;
  }
  model = new THREE.Group();
  appearance = new CardAppearance();
  decryption = new DecryptionController();
  cursor = new THREE.Vector2();
  raycaster = new THREE.Raycaster();
  dummy = new THREE.Object3D();
  cells = [];
  selectedCell = { lane: 2, row: 12 };
  looping = false;
  coordinateOrigin = { lane: 0, row: 0 };
  lift = { value: 0, velocity: 0 };
  rail = { value: 0, velocity: 0 };
  shoulder = { value: 12, velocity: 0 };
  laneFocus = { value: 2, velocity: 0 };
  columnCamera = { value: 0, velocity: 0 };
  returnY = null;
  canInspect = false;
  clearance = 0;
  pulseGain = 1;
  ...
  dragging = false;
  hoverCell = null;
  hoverLifts = /* @__PURE__ */ new Map();
  archiveDrag = new ArchiveDrag();
  dragTrack = null;
  navigatingDrag = false;
  archiveMomentum = null;
  holdingArchive = false;
  cancelPointer = () => {
  };
  rotation = 0;
  targetRotation = 0;
  ...
  labelMark = new Image();
  reduced = false;
  quality = normalizeQuality(void 0);
  appliedQuality = "";
  smaa = new SMAAPass();
  aoKernelSize = 32;
  displayHeight = 0;
  ...
  async load(assetUrl = publicAsset("assets/archive-cassette.glb")) {
    ...
    this.labelTexture.anisotropy = this.renderer.capabilities.getMaxAnisotropy();
  ...
  async createAssemblyModel() {
    this.assemblyTemplate ??= new GLTFLoader().loadAsync(publicAsset("assets/archive-assembly.glb")).then((gltf) => {
      gltf.scene.updateMatrixWorld(true);
      return gltf.scene;
    }).catch((error) => {
      this.assemblyTemplate = void 0;
    ...
    template.traverse((object) => {
      ...
      const name = object.material.name.replace(
        /\.\d+$/,
        ""
      );
    ...
    return {
      model,
      setClarity: (value) => this.appearance.setClarity(model, value),
      dispose: () => {
        for (const mesh of meshes) {
  ...
  setMode(mode) {
    this.cancelPointer();
    this.setHover(null);
    if (mode === "detail") this.decryption.enter(this.scanBlend > 0.9 && this.decryption.clarity > 0.999);
  ...
  setQuality(value) {
    const quality = typeof value === "boolean" ? normalizeQuality(void 0, value) : normalizeQuality(value);
  ...
  rebaseCoordinates() {
    const shift = {
      lane: Math.abs(this.selectedCell.lane) > 2048 ? Math.round((this.selectedCell.lane - 2) / 5) * 5 : 0,
      row: Math.abs(this.selectedCell.row) > 2048 ? Math.floor((this.selectedCell.row - 12) / 8) * 8 : 0
    };
  ...
  select(index, navigation) {
    ...
    const cell = this.looping ? selectionCell(index, this.selectedCell, navigation) : { lane: canonical.lane, row: canonical.row };
    const changed = !sameCell(cell, this.selectedCell);
    if (this.looping && changed && this.loaded && this.lift.value > 1e-4) {
  ...
  drawLabel(index) {
    ...
    c.fillStyle = "#171713";
    c.fillRect(12, 12, 1e3, 6);
    c.fillRect(12, 419, 1e3, 3);
  ...
  resize() {
    ...
    const kind = this.container.closest("[data-layout]")?.dataset.layout ?? "";
    ...
    this.container.dataset.renderQuality = JSON.stringify({
      ...
      shadows: this.renderer.shadowMap.enabled ? this.light.shadow.mapSize.x : 0,
  ...
  canBrowse() {
    return this.presenceTarget === 1 && this.looping && !this.targetDetail && this.detail < 0.2 && this.reveal >= 0.8 && this.loaded && !this.container.closest("[inert]");
  }
  setHover(cell) {
    if (!cell && !this.hoverCell || cell && this.hoverCell && sameCell(cell, this.hoverCell))
      return;
  ...
  pickCell(x, y) {
    ...
    if (!hit) return null;
    if (hit.instanceId !== void 0) return { ...this.drawnCells[hit.instanceId] };
  ...
  trackCoordinate(axis, value) {
    return axis === "lane" ? value / COLUMN_SPACING + 2 : (-value - 2.17) / ROW_SPACING + 15.5;
  }
  dragProjection() {
    ...
    const project = (motion) => {
      const from = center.clone().addScaledVector(motion, -0.5).project(this.camera);
      const to = center.clone().addScaledVector(motion, 0.5).project(this.camera);
      return {
  ...
  trackPosition(axis, coordinate) {
    return axis === "lane" ? (coordinate - 2) * COLUMN_SPACING : -2.17 - (coordinate - 15.5) * ROW_SPACING;
  }
  navigatePlane(coordinate) {
  ...
  bindPointer() {
    ...
    let previousX = 0, startX = 0, startY = 0, moved = false;
    let browse = false, cancelled = false;
    let startTrack = { lane: 0, row: 0 };
    let wheelTotal = 0, wheelTime = 0;
    const pointers = /* @__PURE__ */ new Set();
    const hover = (e) => {
      if (e.pointerType !== "mouse" || !this.canBrowse() || this.archiveMomentum)
        return;
    ...
    canvas.addEventListener("pointermove", (e) => {
      ...
      if (this.dragging && this.canInspect) {
        this.targetRotation = THREE.MathUtils.clamp(
          this.targetRotation + (e.clientX - previousX) * 4e-3,
          -0.8,
          0.8
        );
        previousX = e.clientX;
    ...
    canvas.addEventListener("pointerup", (e) => {
      ...
      if (this.relayActive) {
        if (!cancelled && !moved) {
          const cell = this.pickCell(e.clientX, e.clientY);
          this.onRelayPick?.(cell ? cellKey(cell) : null);
        }
        reset();
        return;
      }
    ...
    canvas.addEventListener(
      ...
      (e) => {
        ...
        if (!this.canBrowse() || activePointer !== null || e.ctrlKey || Math.abs(e.deltaX) > Math.abs(e.deltaY))
          return;
        e.preventDefault();
        if (this.archiveMomentum) this.stopMomentum();
        const now = performance.now();
        const delta = THREE.MathUtils.clamp(
          e.deltaY * (e.deltaMode === 1 ? 40 : e.deltaMode === 2 ? canvas.clientHeight : 1),
          -300,
          300
        );
  ...
  update(time, cinematic) {
    ...
    this.reveal = cinematic ? cinematic.reveal : THREE.MathUtils.lerp(this.reveal, this.targetReveal, blend);
    this.rotation = this.targetDetail ? THREE.MathUtils.lerp(this.rotation, this.targetRotation, blend) : returnStep(this.rotation, dt, this.reduced);
    ...
    for (const [key, value] of this.hoverLifts) {
      ...
      if (target === 0 && next < 1e-4) this.hoverLifts.delete(key);
      else this.hoverLifts.set(key, next);
    }
    const hoverLift = (cell) => this.hoverLifts.get(cellKey(cell)) ?? 0;
    ...
    const idle = !cinematic && !this.reduced && this.targetReveal > 0 && !this.targetDetail && this.detail < 0.01 && this.returnY === null && !aligningCopy && time - this.lastInteraction > 2.5;
    this.idleGain = cinematic ? 0 : THREE.MathUtils.lerp(
      this.idleGain,
    ...
    const indexDim = (lift) => this.selectedIndexOnly ? 1 - ease(lift / 0.4) * (1 - this.flatMix) : this.flatMix;
    ...
    for (const [key, height] of this.relayLifts) {
      const next = height + ((key === gameTarget ? 0.95 : 0) - height) * (this.reduced ? 1 : 1 - Math.exp(-dt * 8));
      if (next < 1e-3 && key !== gameTarget) this.relayLifts.delete(key);
      else this.relayLifts.set(key, next);
    }
    const spectrumPoint = new THREE.Vector3();
    const screenX = (row2, lane2) => {
      spectrumPoint.set((lane2 - 2) * COLUMN_SPACING - trackX, -4.6, (row2 - 15.5) * ROW_SPACING + this.rail.value).project(this.camera);
      return (spectrumPoint.x + 1) / 2;
    };
    const field = (row2, lane2) => {
      if (cinematic)
      ...
      const height = archiveWave(
        row2 + this.coordinateOrigin.row,
        lane2 + this.coordinateOrigin.lane,
        this.scanTime
      ) * this.scanBlend;
      ...
      if (!cinematic && !this.reduced) {
        ...
        for (const p of this.pulses) {
          const distance3 = Math.hypot(row2 - p.row, (lane2 - p.lane) * 2.2);
          const age = time - p.time;
          ripple += this.selectionPulse(distance3, age) * (this.deferSelectionPulse ? rippleEnvelope(distance3, age) : 1);
        }
        pulseHeight = THREE.MathUtils.clamp(ripple, -0.6, 0.6) * this.pulseGain;
      }
      const distance2 = row2 - this.shoulder.value;
      return (height + settlingWave(distance2, 26.56) * columnStrength(lane2, this.laneFocus.value)) * (1 - this.flatMix) + breathing + pulseHeight + (activePlay && !this.reduced ? rhythmDisplacement(row2, lane2, time, play.bands, play.strength, rhythm, screenX(row2, lane2)) : 0) + (this.relayLifts.get(cellKey({ row: row2, lane: lane2 })) ?? 0);
    };
    ...
    if (!cinematic) {
      if (this.returnY !== null && this.rotation !== 0) {
        ...
        damp(
          this.lift,
          this.targetDetail ? INSPECTION_LIFT : this.outgoing.some(
            (o) => o.returnY !== null && o.cell.lane === selectedLane && Math.abs(o.cell.row - selectedRow) < 5
          ) ? 0 : 0.4 * this.targetReveal * (1 - this.flatMix),
          this.reduced ? 35 : this.deferSelectionPulse && !this.targetDetail && this.lift.value < 0.4 ? 7.6 : 4.2,
          dt
        );
      }
    }
    const cameraTarget = this.targetDetail ? ease((this.lift.value - 0.8) / 2.4) : this.returnY !== null ? this.detail : ease((this.lift.value - 0.4) / (INSPECTION_LIFT - 0.4));
    this.detail = cinematic ? cinematic.zoom : THREE.MathUtils.lerp(this.detail, cameraTarget, blend);
    const detail = this.detail;
    this.decryption.update(
      dt,
      detail > 0.78 && this.lift.value > 3.3,
      this.reduced,
      cinematic ? shot + 5 : void 0
    );
    ...
    const entryZ = cinematic ? -23 * (1 - entranceTime) ** 2 : -28 * (1 - entry);
    for (let i = this.outgoing.length - 1; i >= 0; i--) {
      ...
      const { row: row2, lane: lane2 } = o.cell;
      o.group.rotation.x = (field(row2 + 0.5, lane2) - field(row2 - 0.5, lane2)) * 0.024 * (1 - detail) * (1 - quality);
      if (o.lift.value < 1e-4 && Math.abs(o.group.rotation.y) < 1e-4) {
        this.scene.remove(o.group);
        this.appearance.dispose(o.group);
        this.outgoing.splice(i, 1);
      }
    }
    if (this.pendingPulse && !cinematic && !this.targetDetail && this.targetReveal) {
      const selectedY = selectedBase + this.lift.value;
      const oldCardsLower = this.outgoing.every(
        (old) => old.cell.lane !== selectedLane || Math.abs(old.cell.row - selectedRow) > 4 || old.group.position.y + 0.015 < selectedY
      );
    ...
    this.model.rotation.set(
      (field(selectedRow + 0.5, selectedLane) - field(selectedRow - 0.5, selectedLane)) * 0.024 * (1 - detail) * (1 - ease(this.lift.value / 0.4)),
    ...
    const responsiveOpening = Boolean(cinematic) && this.container.closest("[data-layout]")?.dataset.layout === "opening";
    const openingAspect = responsiveOpening ? this.container.clientWidth / this.container.clientHeight / (16 / 9) : 1;
    const openingSpan = (value) => value / Math.min(1, openingAspect);
    ...
    if (cinematic) {
      ...
      const shotYaw = yaw - THREE.MathUtils.degToRad(9 * earlyTurn + 32 * finalTurn);
      const shotElevation = elevation - THREE.MathUtils.degToRad(1.5 * earlyTurn + 3.7 * finalTurn);
      viewDirection.set(
      ...
      viewDirection.lerp(new THREE.Vector3(-0.277, 0.238, 0.931), detail).normalize();
    }
    if (cinematic) {
    ...
    if (cinematic && shot >= 25.05 && shot <= 27.3) {
      ...
      const anchorAim = this.model.position.clone().add(new THREE.Vector3(-2.5, 3.7, 0));
    ...
    if (cinematic && shot > 27.3) {
      ...
      const screenX2 = THREE.MathUtils.lerp(
      ...
      const up = new THREE.Vector3().crossVectors(viewDirection, right).normalize();
      const anchorAim = this.model.position.clone().add(new THREE.Vector3(-2.5, 3.7, 0));
      anchorAim.addScaledVector(right, -(screenX2 - 960) * openingAspect / pixelScale);
      anchorAim.addScaledVector(up, -(540 - screenY) / pixelScale);
      cameraAim.lerp(anchorAim, ease((shot - 27.3) / 0.5));
    }
    const framing = archiveFraming(
      this.container.clientWidth,
      this.container.clientHeight,
      span,
      detail,
      this.container.closest("[data-layout]")?.dataset.layout === "compact"
    );
    if (!cinematic) {
      const right = new THREE.Vector3().crossVectors(new THREE.Vector3(0, 1, 0), viewDirection).normalize();
      const up = new THREE.Vector3().crossVectors(viewDirection, right).normalize();
      ...
      const detailAim = this.model.position.clone().add(new THREE.Vector3(0, 1.85, 0));
    ...
    this.cells = fixed ? Array.from({ length: 160 }, (_, i) => poolCell(i)) : this.visibility.update(this.camera, fog.far, trackX, entryZ + this.rail.value, this.extraCoverage);
    ...
    for (const cell of this.cells) {
      const { row: row2, lane: lane2 } = cell;
      if (hidden.has(cellKey(cell))) continue;
      const x = (lane2 - 2) * COLUMN_SPACING - trackX;
      const y = -4.6 + field(row2, lane2) + hoverLift(cell) - this.presentationDrop(cell);
      const z = (row2 - 15.5) * ROW_SPACING + entryZ + this.rail.value;
      ...
      const slope = field(row2 + 0.5, lane2) - field(row2 - 0.5, lane2);
    ...
    const lane = selectedLane, row = selectedRow;
    ...
    this.canInspect = !cinematic && Boolean(this.targetDetail) && detail > 0.9 && this.pulseGain < 0.01 && this.clearance > 0.3;
    this.container.dataset.inspection = this.returnY !== null ? "aligning" : this.canInspect ? "ready" : this.targetDetail ? "lifting" : "preview";
    const focalPoint = this.model.position.clone().add(new THREE.Vector3(0, 2, 0)).applyMatrix4(this.camera.matrixWorldInverse);
    const bokehUniforms = this.bokeh.uniforms;
    bokehUniforms.focus.value = -focalPoint.z;
    bokehUniforms.aperture.value = THREE.MathUtils.lerp(3e-4, 8e-4, detail) * this.quality.depthOfField / 100;
  ...
  projectCard(x, y) {
    this.model.updateMatrixWorld(true);
    const p = this.model.localToWorld(new THREE.Vector3(x, y, 0.255)).project(this.camera);
  ...
  getStats() {
    this.model.updateMatrixWorld(true);
    const project = (x, y, z) => {
      const p = this.model.localToWorld(new THREE.Vector3(x, y, z)).project(this.camera);
    ...
    return {
      ...
      modelPosition: this.model.position.toArray().map((v) => Math.round(v * 1e4) / 1e4),
      cameraPosition: this.camera.position.toArray().map((v) => Math.round(v * 1e4) / 1e4),
      ...
      selectionPhase: this.pendingPulse ? "lifting" : this.pulses.length ? "wave" : "settled",
      ...
      extraction: Math.round(this.lift.value * 1e3) / 1e3,
      appearance: Math.round(ease(this.lift.value / 0.4) * 1e3) / 1e3,
      cameraDetail: Math.round(this.detail * 1e3) / 1e3,
      ...
      rail: Math.round(this.rail.value * 1e3) / 1e3
    };
  }
}
export {
